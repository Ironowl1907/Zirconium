...
OpenGLShader::OpenGLShader(const std::string& filepath)
...

    {
        ...
        CreateProgram();
        ZR_CORE_WARN("Shader creation took {0} ms", timer.ElapsedMillis());
    }
...
std::string OpenGLShader::ReadFile(const std::string& filepath) {
    ...
    if (in) {
        ...
        if (size != -1) {
            ...
            ZR_CORE_ERROR("Could not read from file '{0}'", filepath);
        }
    } else {
...
std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source) {
    ...
    while (pos != std::string::npos) {
        ...
        ZR_CORE_ASSERT(eol != std::string::npos, "Syntax error");
        size_t begin = pos + typeTokenLength + 1; // Start of shader type name (after "#type " keyword)
        std::string type = source.substr(begin, eol - begin);
        ZR_CORE_ASSERT(Utils::ShaderTypeFromString(type), "Invalid shader type specified");

        size_t nextLinePos =
            source.find_first_not_of("\r\n", eol); // Start of shader code after shader type declaration line
        ZR_CORE_ASSERT(nextLinePos != std::string::npos, "Syntax error");
...
void OpenGLShader::CompileOrGetVulkanBinaries(const std::unordered_map<GLenum, std::string>& shaderSources) {
    [[maybe_unused]] GLuint program = glCreateProgram();
    ...
    for (auto&& [stage, source] : shaderSources) {
        ...
        if (in.is_open()) {
            ...
            if (module.GetCompilationStatus() != shaderc_compilation_status_success) {
                ZR_CORE_ERROR(module.GetErrorMessage());
                ZR_CORE_ASSERT(false, "");
            }
...
void OpenGLShader::CreateProgram() {
    ...
    if (isLinked == GL_FALSE) {
        ...
        ZR_CORE_ERROR("Shader linking failed ({0}):\n{1}", m_FilePath, infoLog.data());
...
void OpenGLShader::Reflect(GLenum stage, const std::vector<uint32_t>& shaderData) {
    ...
    ZR_CORE_TRACE("OpenGLShader::Reflect - {0} {1}", Utils::GLShaderStageToString(stage), m_FilePath);
    ZR_CORE_TRACE("    {0} uniform buffers", resources.uniform_buffers.size());
    ZR_CORE_TRACE("    {0} resources", resources.sampled_images.size());

    ZR_CORE_TRACE("Uniform buffers:");
    for (const auto& resource : resources.uniform_buffers) {
        ...
        ZR_CORE_TRACE("  {0}", resource.name);
        ZR_CORE_TRACE("    Size = {0}", bufferSize);
        ZR_CORE_TRACE("    Binding = {0}", binding);
        ZR_CORE_TRACE("    Members = {0}", memberCount);
    }
}
